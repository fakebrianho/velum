...
class Router {
	...
	_onLinkClick(e) {
		...
		e.preventDefault()
		this.navigate(href, { pushState: true, trigger: 'link', link })
	}

	_onPopState() {
		const url = window.location.pathname + window.location.search
		this.navigate(url, { pushState: false, trigger: 'popstate', link: null })
	}
	...
	getRouteForNamespace(namespace) {
		...
		const byNamespace = entries.find(([, config]) => config.namespace === namespace)
	...
	// Measure the incoming container's natural block width by briefly placing it
	// in normal flow (visibility:hidden so it's never painted). This gives us the
	// width the element will have AFTER the transition so we can pin the
	// absolutely-positioned stage to the same value — clearing it later causes no jump.
	_measureNaturalWidth(container, parent) {
	...
	_createStageElements(fromContainer, toContainer) {
		...
		const naturalWidth = this._measureNaturalWidth(toContainer, parent)
		...
		toContainer.style.width = `${naturalWidth}px`
		toContainer.style.opacity = '0'
		toContainer.style.pointerEvents = 'none'

		fromContainer.after(toContainer)

		return { parent, previousParentPosition, stagedContainer: toContainer }
	}
	...
	_commitNavigation({ fromContainer, toContainer, parent, previousParentPosition, url, pushState }) {
	...
	async navigate(url, { pushState = true, trigger = 'programmatic', link = null } = {}) {
		...
		try {
			...
			const toRouteEntry =
				this.getRouteForNamespace(toNamespace) || this.getRouteForUrl(page.url)
