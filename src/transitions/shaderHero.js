		...
		run({ from, to, sharedElements }) {
			return new Promise((resolve) => {
				...
				if (!hasHero) {
					...
					if (from)
						tl.to(from, { opacity: 0, duration: fadeDuration })
					tl.to(to, { opacity: 1, duration: fadeDuration })
					return
				}

				// No onComplete — we call resolve() explicitly once the vertex
				// animation finishes, letting the router reveal `to` at its
				// correct natural-flow position with no jump or flash.
				...
				for (const {
					...
					// Hide the real fromEl — the WebGL plane stands in for it.
					// Do NOT touch toEl's opacity: it's already hidden by its
					// parent container (to) being opacity:0. When the router
					// sets temp.style.opacity='1' and clears absolute positioning
					// after resolve(), toEl appears at its correct position.
