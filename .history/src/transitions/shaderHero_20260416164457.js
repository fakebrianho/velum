	...
	return {
		run({ from, to, sharedElements }) {
			return new Promise((resolve) => {
				...
				gsap.set(to, { opacity: 0 })
				// Fade out old content alongside the vertex animation
				if (from) tl.to(from, { opacity: 0, duration: fadeDuration }, 0)

				// Once vertex animation ends: remove WebGL meshes, reveal shared
				// elements in DOM, then fade in the incoming container
				tl.add(() => {
				...
				tl.to(to, { opacity: 1, duration: fadeDuration }, duration)
			})
		},
	}
}
